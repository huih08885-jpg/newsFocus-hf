...
-- 创建索引
CREATE INDEX IF NOT EXISTS idx_lottery_results_period ON lottery_results(period);
CREATE INDEX IF NOT EXISTS idx_lottery_results_date ON lottery_results(date DESC);

-- 添加注释
COMMENT ON TABLE lottery_results IS '福利彩票开奖结果表';
COMMENT ON COLUMN lottery_results.period IS '期号';
COMMENT ON COLUMN lottery_results.date IS '开奖日期';
COMMENT ON COLUMN lottery_results.red_balls IS '红球号码数组';
COMMENT ON COLUMN lottery_results.blue_ball IS '蓝球号码';
COMMENT ON COLUMN lottery_results.url IS '详情页URL';
